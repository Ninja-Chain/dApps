...
export interface NativeMultiSendInstance {
  ...
  // actions
  transfer: (sender: string, recipient: Recipient[]) => Promise<string>;
}
...
export const NativeMultiSend = (client: SigningCosmWasmClient): NativeMultiSendContract => {
  const use = (contractAddress: string): NativeMultiSendInstance => {
    ...
    const transfer = async (sender: string, recipients: Recipient[]): Promise<string> => {
      const result = await client.execute(sender, contractAddress, { send: { recipients } });
      return result.transactionHash;
    };
